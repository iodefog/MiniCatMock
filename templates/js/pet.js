...
// （抽屉用 ai-nl-path）；再不行则从指令输入框(ai-nl-rule-input)或
// AI 助手 prompt 输入框里抽取路径 token，避免用户把路径填在别处时仍被提示"需要输入 path"。
function resolveEndpointPath(preferredId) {
    const candidates = [preferredId, 'ai-nl-path'];
...
async function replayInterfaceViaCurl(pathId, outputId, statusId) {
    ...
    await _doReplayInterfaceViaCurl(path, outputId, statusId);
}

// 直接使用路径字符串（不再依赖 DOM 输入框）
async function _doReplayInterfaceViaCurl(path, outputId, statusId) {
...
async function askAIAnalysis(question, cfg, outputEl, statusEl, isDrawer) {
    ...
    const pathEl = document.getElementById(isDrawer ? 'ai-nl-path' : 'global-ai-prompt');
    const path = pathEl ? (isDrawer ? pathEl.value.trim() : extractPathFromPrompt(pathEl.value.trim())) : '';
    if (path) {
...
async function runGlobalAIGenerate() {
    ...
    // 从需求描述中自动提取形如 sf001/list、/api/v1/user 的路径，
    // 提取不到才提示用户手动输入。
    let pathInput = extractPathFromPrompt(prompt);
    ...
    if (isReplayLike(prompt)) {
        const replayPath = extractPathFromPrompt(prompt);
        if (!replayPath) {
            showToast((typeof currentLang !== 'undefined' ? currentLang : 'zh') === 'zh' ? '⚠️ 请先在需求描述中包含接口路径（如 /api/v1/user）' : '⚠️ Please include the endpoint path in description', '#f59e0b');
            return;
        }
        await _doReplayInterfaceViaCurl(replayPath, 'global-ai-stream-preview', 'global-ai-gen-status');
        return;
    }
...
3. 默认外层结构为 {"code": 200, "message": "success", "data": ...}。`;
    ...
    try {
        ...
        globalAiGeneratedPath = pathInput;
    } catch (err) {
        previewEl.classList.remove('streaming');
        generateBtn.disabled = false;
        generateBtn.textContent = (typeof currentLang !== 'undefined' ? currentLang : 'zh') === 'zh' ? '🚀 生成规则' : '🚀 Generate Rule';
...
function discardGlobalAIResult() {
    ...
    globalAiGeneratedText = '';
    globalAiGeneratedPath = '';
}

async function saveGlobalAIResult() {
    if (!globalAiGeneratedText) return;
    const pathInput = globalAiGeneratedPath;
    const method = 'POST';

    if (!pathInput) {
        showToast((typeof currentLang !== 'undefined' ? currentLang : 'zh') === 'zh' ? '⚠️ 无法识别接口路径，请确保需求描述中包含路径（如 /api/v1/user）' : '⚠️ Cannot identify path, please include it in description', '#f59e0b');
        return;
    }
...
function updateGlobalAIBadge() {
    ...
    if (cfg.apiKey) {
        const providerName = cfg.provider === 'deepseek' ? 'DeepSeek' : (cfg.provider === 'openai' ? 'OpenAI' : (cfg.provider === 'custom' ? (lang === 'zh' ? '自定义' : 'Custom') : cfg.provider.toUpperCase()));
        statusText = `${providerName} ⚙️`;
    }
