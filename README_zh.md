...
当手机与 Mac 处于**不同网络**（4G/5G、跨网段，或无法连接同一 Wi-Fi）时，局域网直连会失效。开启「跨网联通（Ngrok）」模式即可建立一条安全的公网隧道，让手机在任何地方都能访问你本地的 Mock 服务。

### 安装 Ngrok（macOS，使用 Homebrew）
```bash
brew install ngrok/ngrok/ngrok
```

### 配置 Authtoken（免费，稳定隧道必需）
在 [ngrok.com](https://ngrok.com) 免费注册并复制你的 authtoken，然后执行：
```bash
ngrok config add-authtoken <你的AUTHTOKEN>
```
> 不配置 authtoken 隧道仍可使用，但会受到 Ngrok 免费版会话数/时长的限制。

### 安全隧道的运作方式
1. 每次开启该模式，服务端都会**随机生成 16 位高强度密码**，并以 `--basic-auth mockuser:<随机密码>` 启动 Ngrok。
2. 公网地址与随机凭据会被编码进 **Web 面板展示的二维码** 中。用手机扫码即可直连——**密码不会经过聊天工具、邮件，也不会嵌入被分享的链接**。
3. 由于每位开发者都运行在自己的 Mac 上，且二维码为本地闭环扫码（Mac ↔ 自己的手机），随机密码**不会离开你的设备生态**，在本机闭环用法下几乎不存在泄露可能。

### 安全须知
* 流量会经由 Ngrok 服务器中转（TLS 在其侧终止），因此**请勿经此通道传输核心保密数据**。
* 该方案定位为**临时、非敏感的跨网调试**用途，**并非生产级**方案。
* 遵循「即用即开、用完即关」原则：调试完成后切回「局域网直连」模式以关闭公网通道。
* 避免将二维码/密码转发至群聊或截图外传；每次启动都会重新生成新的随机密码。
