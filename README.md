# 🐱 小猫Mock (Little Cat) — 极速无线 Mock & 抓包调试服务器

![功能示意图](小猫Mock截图/小猫mock-功能.png)

「小猫Mock」是一款专为移动端（iOS / Android）开发者量身打造的**零配置、免代理、扫码即连**的轻量级无线抓包与 Mock 调试服务器。
...
在立项初期，我们调研了市面上主流的各类网络调试与 Mock 方案：

| 方案类别 | 代表工具 | 痛点与局限性 |
| :--- | :--- | :--- |
| **传统中间人抓包代理** | Charles / Fiddler / Proxyman | 强依赖系统全局 HTTP 代理配置；iOS 10+ 与 Android 7+ 针对证书信任限制严苛，HTTPS 解析门槛高；规则配置繁琐，不易团队共享。 |
| **标准后端 Mock 平台** | YApi / Apifox / Postman | 主要解决“从无到有”的跑通问题。多人共用一套环境容易互相覆盖（环境污染）；无法做到对真实业务数据的局部动态篡改，无法抓包。 |
| **纯端侧本地 Mock** | GYHttpMock 等硬编码拦截库 | Mock 逻辑需要写死在 App 源码里，修改规则需要重新编译运行，缺乏可视化的动态下发与即时响应能力。 |

**结论与抉择**：
为了解决“后期深度测试”与“配置摩擦”的核心痛点，我们决定采用 **控制平面（Web 大屏） + 数据平面（App 内局部网络劫持）分离** 的创新架构。即在 App 端只做局域网地址重定向（绕过系统代理机制），把所有抓包和规则判定的重活交给「小猫Mock」桌面端引擎。
